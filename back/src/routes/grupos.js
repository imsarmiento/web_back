...
router.get("/:id", async (req, res) => {
    try {
        ...
        if (!grupo) {
            return res.status(404).send({error: "No existe un grupo con el id especificado"});
        }
...
router.patch("/:idG/eventos/:idE", async (req, res) => {
    ...
    try {
        ...
        if (!evento) {
            return res.status(404).send({error: "No existe un evento con el id especificado"});
        }
...
router.delete("/:id", async (req, res) => {
    try {
        ...
        if (!grupo) {
            return res.status(404).send({error: "No existe un grupo con el id especificad"});
        }
        res.send(grupo);
    } catch (error) {
