    ...
    for (let i = 0; i < size; i++) {
      ...
      if (!usuario) {
        return res.status(404).send({
          error: "No existe un usuario con el correo " + usuariosParam[i],
        });
      }
        ...
        if (eventoParam.frecuencia === "sinRepetir") {
          ...
          if (reglaFin <= eventos[j].end && reglaFin >= eventos[j].start) {
            return res.status(500).send({
              error:
                "Existe un conflicto de horario con una o mas personas para esta fecha",
            });
          } else if (
            reglaInicio <= eventos[j].end &&
            reglaInicio >= eventos[j].start
          ) {
            return res.status(500).send({
              error:
                "Existe un conflicto de horario con una o mas personas para esta fecha",
...
router.post("/verificarDisponibilidad", async (req, res) => {
  try {
    ...
    for (let i = 0; i < size; i++) {
      ...
      res
        .status(200)
        .send({ mensaje: "No existen conflictos de horario para este evento" });
    }
  } catch (error) {
    return res.status(400).send({ error: error });
  }
